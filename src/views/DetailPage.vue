...
function showLightbox() {
  const currentIndex = props.product.images.findIndex(
...
<template>
  <div className="container mx-auto px-4">
    ...
    >
      <div className="gallery-container">
        ...
        >
          ...
          <div
            v-else
            className="absolute top-0 left-0 w-full h-full flex items-center justify-center text-gray-500 font-medium"
          >
            No Image
          </div>
        </div>
        ...
        >
          <img
            ...
            :className="{
              ...
              'opacity-70': !(selectedImage && selectedImage.id === img.id),
            }"
    ...
    <div v-if="otherProducts.length > 0" class="mt-12 text-center">
      <hr class="border-0 h-px bg-gray-300 mb-8" />
      <h3 class="mb-6 text-xl font-semibold">Produk Lainnya</h3>
      <div
        class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-5 lg:grid-cols-5 gap-4 md:gap-6 mx-auto mb-8"
      >
        <ProductCard
          v-for="other_product in otherProducts"
          :key="other_product.id"
